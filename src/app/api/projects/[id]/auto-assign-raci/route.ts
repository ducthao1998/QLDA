...
export async function POST(
  ...
  try {
    ...
    const availableUsers: AlgoUser[] = projectMembers.map((pm: any) => {
      return {
        id: pm.id,
        name: pm.full_name,
    ...
    const algorithmTasks: AlgoTask[] = tasksData.map((task: any) => ({
      id: task.id,
      name: task.name,
    ...
    if (planning) {
      ...
      assignments = algorithmTasks.map(t => {
        ...
        for (const u of availableUsers) {
          const exps = t.required_skills.map((sid: number) => (experienceMatrix?.[u.id]?.[sid] ?? 0))
