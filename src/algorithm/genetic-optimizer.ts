interface GeneticOptimizerParams {
  ...
  function crossover(selectedIndividuals: any[], crossoverRate: number) {
    ...
    offspring.push(...selectedIndividuals.map((individual) => individual.schedule))
    ...
    for (let i = 0; i < selectedIndividuals.length / 2; i++) {
      if (Math.random() < crossoverRate) {
        ...
        const [child1, child2] = crossoverSchedules(parent1, parent2)
  ...
  // Lai ghép hai lịch trình
  function crossoverSchedules(parent1: any[], parent2: any[]) {
    // Tạo bản sao của các công việc
    const child1 = parent1.map((task) => ({ ...task }))
    const child2 = parent2.map((task) => ({ ...task }))
